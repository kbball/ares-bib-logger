...
func TestProcessMessage_Checkpoint_PublishesReplyAndDoesNotLogBib(t *testing.T) {
	...
	a.processMessage(context.Background(), jsonMsg("NODE1: checkpoint", 2))
	...
	var ack outgoingMsg
	require.NoError(t, json.Unmarshal(pub.published[0].payload, &ack))
	assert.Equal(t, "GDR AS6", ack.Message)
}
...
func TestProcessMessage_Count_PublishesReply(t *testing.T) {
	...
	a.processMessage(context.Background(), jsonMsg("NODE1: count", 2))
	...
	var ack outgoingMsg
	require.NoError(t, json.Unmarshal(pub.published[0].payload, &ack))
	assert.Equal(t, "logged 15 at AS6", ack.Message)
}
...
func TestProcessMessage_Search_PublishesReply(t *testing.T) {
	...
	a.processMessage(context.Background(), jsonMsg("NODE1: search smith", 2))
	...
	var ack outgoingMsg
	require.NoError(t, json.Unmarshal(pub.published[0].payload, &ack))
	assert.Equal(t, "1 match(es): 100 Alice Smith (GDR)", ack.Message)
}
...
func TestProcessMessage_Dup_PublishesReply(t *testing.T) {
	...
	a.processMessage(context.Background(), jsonMsg("NODE1: dup 100", 2))
	...
	var ack outgoingMsg
	require.NoError(t, json.Unmarshal(pub.published[0].payload, &ack))
	assert.Equal(t, "100 Alice Smith: already logged at AS6 14:32", ack.Message)
}
...
func TestProcessMessage_Help_PublishesHelpText(t *testing.T) {
	...
	a.processMessage(context.Background(), jsonMsg("NODE1: help", 2))
	...
	var ack outgoingMsg
	require.NoError(t, json.Unmarshal(pub.published[0].payload, &ack))
	assert.Equal(t, meshutil.HelpText, ack.Message)
}
