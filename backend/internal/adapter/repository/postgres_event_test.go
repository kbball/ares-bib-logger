...
var eventCols = []string{"id", "name", "archived", "winlink_blank_line_after_header", "created_at"}

func TestEventRepo_List_ReturnsRows(t *testing.T) {
	db, mock := newMock(t)
	now := time.Now()

	mock.ExpectQuery(qe(`SELECT id, name, archived, winlink_blank_line_after_header, created_at FROM events WHERE NOT archived ORDER BY created_at DESC`)).
		WillReturnRows(sqlmock.NewRows(eventCols).
			AddRow(1, "Event A", false, false, now).
			AddRow(2, "Event B", false, false, now))
...
func TestEventRepo_Get_Found(t *testing.T) {
	...
	mock.ExpectQuery(qe(`SELECT id, name, archived, winlink_blank_line_after_header, created_at FROM events WHERE id = $1`)).
		WithArgs(1).
		WillReturnRows(sqlmock.NewRows(eventCols).AddRow(1, "GA Death Race", false, false, now))
...
func TestEventRepo_Create_Success(t *testing.T) {
	...
	mock.ExpectQuery(qe(`INSERT INTO events (name) VALUES ($1) RETURNING id, name, archived, winlink_blank_line_after_header, created_at`)).
		WithArgs("New Event").
		WillReturnRows(sqlmock.NewRows(eventCols).AddRow(5, "New Event", false, false, now))
...
func TestEventRepo_SetWinlinkBlankLineAfterHeader_Success(t *testing.T) {
	db, mock := newMock(t)

	mock.ExpectExec(qe(`UPDATE events SET winlink_blank_line_after_header = $1 WHERE id = $2`)).
		WithArgs(true, 3).
		WillReturnResult(sqlmock.NewResult(0, 1))

	err := repository.NewEventRepo(db).SetWinlinkBlankLineAfterHeader(context.Background(), 3, true)
	assert.NoError(t, err)
	assert.NoError(t, mock.ExpectationsWereMet())
}

func TestEventRepo_SetWinlinkBlankLineAfterHeader_Error(t *testing.T) {
	db, mock := newMock(t)
	...
	err := repository.NewEventRepo(db).SetWinlinkBlankLineAfterHeader(context.Background(), 3, false)
	assert.ErrorContains(t, err, "db down")
	assert.NoError(t, mock.ExpectationsWereMet())
}
