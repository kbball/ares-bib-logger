...
const cpCols = `id, race_id, code, display_name, column_name, display_order, distance_from_start, cutoff_time, created_at`

func scanCheckpoint(s interface{ Scan(...any) error }) (entity.Checkpoint, error) {
	var cp entity.Checkpoint
	err := s.Scan(&cp.ID, &cp.RaceID, &cp.Code, &cp.DisplayName, &cp.ColumnName, &cp.DisplayOrder, &cp.DistanceFromStart, &cp.CutoffTime, &cp.CreatedAt)
	return cp, err
}
...
func (r *CheckpointRepo) Update(ctx context.Context, cp entity.Checkpoint) (entity.Checkpoint, error) {
	updated, err := scanCheckpoint(r.db.QueryRowContext(ctx,
		`UPDATE checkpoints SET code = $1, display_name = $2, distance_from_start = $3, column_name = $4, cutoff_time = $5 WHERE id = $6 RETURNING `+cpCols,
		cp.Code, cp.DisplayName, cp.DistanceFromStart, cp.ColumnName, cp.CutoffTime, cp.ID))
	if err != nil {
		return entity.Checkpoint{}, mapNotFound(err)
	}
	return updated, nil
...
func (r *CheckpointRepo) Create(ctx context.Context, cp entity.Checkpoint) (entity.Checkpoint, error) {
	created, err := scanCheckpoint(r.db.QueryRowContext(ctx,
		`INSERT INTO checkpoints (race_id, code, display_name, display_order, distance_from_start, column_name, cutoff_time)
		 VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING `+cpCols,
		cp.RaceID, cp.Code, cp.DisplayName, cp.DisplayOrder, cp.DistanceFromStart, cp.ColumnName, cp.CutoffTime))
	if err != nil {
