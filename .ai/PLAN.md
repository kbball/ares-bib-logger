...
- Admin accordion open/closed state now persists across page visits via `localStorage`, independently for "Setup" and "Edit Runners" (`Accordion` made controlled with `expanded`/`onChange`, `openAccordions: Set<string>` state seeded from storage on mount)
...
- Winlink export footer for post-roster-lock additions: migration 000009 adds `Race.RosterCount` (size of the roster at lock time, set by `RunnerService.ImportRoster` via an updated `RaceRepository.LockRoster(id, rosterCount)`; backfilled for existing locked races from their current runner count) and `Race.WinlinkFooterRows` (editable any time via new `RaceRepository.SetWinlinkFooterRows`/`RaceService.SetWinlinkFooterRows`/`PUT /api/races/{id}/winlink-footer-rows`, independent of roster lock, so an RD can raise it mid-race). `WinlinkService.Export` now splits a race's runners into the original roster (`SortOrder <= RosterCount`) and post-lock additions (transfers, late adds), writes the original list unchanged, then pads/writes the footer to `WinlinkFooterRows` — never dropping overflow additions, just reporting the excess as `WinlinkExportResult.FooterOverflowCount` (surfaced via an `X-Footer-Overflow-Count` response header and a warning `Alert` on the Winlink Export tab) — followed by a closing footer marker line repeating the checkpoint header (only emitted when `WinlinkFooterRows > 0`), so the operator can visually confirm the column is complete; the top header line now reuses the existing `checkpointHeader` helper instead of duplicating its ColumnName/DisplayName fallback inline. Admin panel's race card gained a "Winlink footer rows" number field.

**v1.6 — 2026-09-19**

- Data Entry's Overall card gained a "Still to come" line: per-race checkpoint-based stats (`through`/`dnsDnf`/`stillToCome`/projected next arrival) were extracted out of the per-race `Paper` card's inline `.map` into a shared `raceStats` array computed once, so the Overall card's total and the per-race cards can never disagree about what "still to come" means for a given race. The total only sums races that have an active checkpoint set (mirroring the per-race card's own `—` for a race with none) — a race without a checkpoint is left out of the sum, not counted as zero; if no race in the event has one, the whole line reads `—` rather than `0`.
...
### ~~0. Persist last-opened admin accordion across navigation~~ ✅ DONE
- Admin page's "Setup" and "Edit Runners" accordions now persist their open/closed state independently to `localStorage` (`ares-bib-logger:admin-accordions-open`, JSON array of open accordion names), fails open (defaults to collapsed) if storage is unavailable — same pattern as dark/light mode persistence
...
- `RunnerService.AddRunner` (new port method) reuses the `MaxSortOrder` + `BulkCreate` pattern from `TransferRace`, with `Status: StatusUnknown` (fresh roster row, not yet seen at a checkpoint); intentionally **not** gated by `RosterLocked` and never calls `LockRoster` — late registrations must work even after the initial roster import; new `POST /api/races/{raceID}/runners` endpoint; Admin panel "Edit Runners" accordion gained an "Add Runner to Roster" section (race select shows all races, unlike Roster Import's locked-race filter)
...
### ~~5. Aid-station keyword compatibility, arrival visibility, and reliability fixes~~ ✅ DONE
- Winlink import now accepts `DROP` as a synonym for `DNF`, and `CHG <race>` as a synonym for `MOVED <race>` — matching the vocabulary aid stations actually use over Winlink. Export now writes `CHG` (not `MOVED`) as the canonical keyword; `DNF` stays canonical for export
- Runners tab per-checkpoint columns now fall back to the runner's overall status (DNS/DNF/MOVED) when no checkpoint log exists for that cell — mirrors the Winlink export's existing fallback, since marking DNS/DNF from Data Entry only updates `Runner.Status` and never writes a `CheckpointLog`
- Runner detail modal now projects arrival at the next checkpoint the runner hasn't reached yet (course order), not just the session's active checkpoint; labeled `(active)` when the two coincide
- New "Not Arrived" filter chip on the Runners tab: runners with no real time log at their race's active checkpoint, excluding DNS/DNF/MOVED/FINISHED (mirrors Data Entry's "still to come" definition)
- Fixed Winlink Export's copy buttons silently failing when `navigator.clipboard` is unavailable (plain-HTTP field deployments aren't a secure context) — added an `execCommand('copy')` fallback via a hidden textarea, with an error message if both paths fail
- Fixed duplicate rows in Data Entry's Recent Log: a manually-submitted bib was pushed once from the direct HTTP response and again from the SSE `bib_logged` broadcast (which echoes to the submitting client too). The frontend now sends a per-submission `X-Request-Id` header, echoed back on the broadcast, so the submitting client can recognize and skip its own echo while still showing logs from other sources (mesh, other stations)
...
| 2026-09-18 | Winlink import accepts `DROP`/`CHG` as synonyms for `DNF`/`MOVED`; export now writes `CHG` (not `MOVED`), but keeps `DNF` (not `DROP`) | Real-world aid-station Winlink traffic uses `DROP` and `CHG`; `DNF` was kept canonical for export since no station feedback asked for that one to change, while `CHG` was explicitly requested as the outbound keyword |
| 2026-09-18 | Runner-status fallback for empty Runners-tab checkpoint cells reuses the exact DNS/DNF/MOVED logic already in `WinlinkService.Export`, rather than writing a checkpoint log when a status changes | Preserves the existing invariant that `CheckpointLog` rows represent real station crossings; the UI reads the same fallback the export already relies on instead of the data model growing a new kind of log entry |
| 2026-09-18 | Data Entry's own bib submissions are deduplicated via a client-generated `X-Request-Id` echoed back on the SSE broadcast, not by suppressing the direct API-driven UI update | Relying on the SSE broadcast alone as the sole source of truth for the submitter's own action risked no on-screen confirmation if the connection briefly dropped right after posting — reliability matters more for a race-timing tool than avoiding one extra header |
| 2026-09-19 | Winlink footer occupants are any runner added after roster lock (transfers + late adds), split from the original roster purely by `SortOrder <= RosterCount` | Both `TransferRace` and `AddRunner` already share the same "append at `MaxSortOrder+1`" mechanism, so there's no structural difference between them worth exposing in the footer split; filtering by `SortOrder` (not slice position) stays correct even if the runner list has gaps |
| 2026-09-19 | `WinlinkFooterRows` is a per-race setting, editable independently of `RosterLocked` | Footer capacity depends on the race/distance (a 50-miler expects more transfers than a 5K), and the RD needs to raise it mid-race as transfers happen — it must never be gated by the same lock that protects the original roster from mid-race overwrites |
| 2026-09-19 | Footer overflow (more additions than `WinlinkFooterRows`) is written in full and only reported, never truncated | A pre-printed paper form running out of blank lines is an operator problem to solve by raising the row count; silently dropping a runner's status from the Winlink packet would be a worse failure than an unexpected extra line |
| 2026-09-19 | `roster_count` backfill for pre-existing locked races uses `COUNT(*)` of current runners, not a historical original-roster size | There's no way to reconstruct which pre-migration runners were "original" vs. already-transferred-in without a new audit trail; grandfathering all existing runners into the original list means the migration itself never creates a false footer-overflow warning — only genuinely new post-migration additions land in the footer |
| 2026-09-19 | Overall card's "Still to come" sums only races with an active checkpoint set, excluding rather than zeroing races without one | Revisits the earlier "no single active-checkpoint concept across races" rationale for the Overall card (2026-08-23): a per-race total *can* be summed once each contributing race supplies its own checkpoint-based figure — the aggregate just has to stay honest about which races it covers, the same way each per-race card already shows `—` instead of `0` when it has no checkpoint |
